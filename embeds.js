...
function escapeData(){
  ...
  .addFields(
    {name: 'Pokemon that can escape in game', value:
...
function helpData(){
  let helpStructure = new MessageEmbed()
    ...
    .addFields(
      { name: '!pokeinfo <pokemonName>', value: // \`!pokemoninfo scizor\` for command box 
    `
    ...
    `, inline: true }, 
      ...
      {name: '!nature', value:`!nature, shows nature chart.`, inline: true },
    
      {name: '!drop', value:`!drop, gives a list of pokemon that may ocasionally drop items.`, inline: true }, 
    
      {name: '!thanks', value:`!thanks, special thanks to those who have helped and are helping the making GyaradosThief (GT)`, inline: true },

      {name: '!pokemove <pokemon>', value: `!pokemove venipede, provides information on the move pokemon can learn.`, inline: true },
    ...
    function thanksData(){
      ...
      function dropData(){
        ...
        `, inline: true },
        {name: '\u200B', value: '\u200B' },
        {name: 'Use the command !pokeinfo to find the item which the pokemon may drop', value:
        `
...
function nonmapData(){
	nonmapStructure = new MessageEmbed()
		...
		.addFields(
			{name: 'These Pokemon are usually found during Events.', value: 
		`
